<!-- Parent: ../AGENTS.md -->
<!-- Generated: 2026-05-21 | Updated: 2026-05-21 -->

# Projects API

## Purpose
Project CRUD operations (list, create, bulk operations)

## Key Files
| File | Description |
|------|-------------|
| `route.ts` | API endpoint. Exports: GET, POST |

## Subdirectories
| Directory | Purpose |
|-----------|----------|
| `[id]/` | Single project operations (read, update) |
| `archived/` | List all archived projects |
| `billing-count/` | Aggregate billing statistics |

## For AI Agents

### Working In This Directory
This is an API endpoint. Uses `requireUser()` for auth gating. Returns JSON via helpers like `ok()`, `badRequest()`, `notFound()`, `unauthorized()`, `serverError()`, or `conflict()`.

## Dependencies

### Internal
- `lib/...` — utilities and shared logic

<!-- MANUAL: -->
